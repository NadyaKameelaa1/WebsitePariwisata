...
// mencari pariwisata berdasarkan keyword
if (!empty($search_query)) {
    $sql_search = "SELECT p.*, k.nama_kategori 
                  FROM pariwisata p 
                  JOIN kategori k ON p.id_kategori = k.id_kategori 
                  WHERE p.nama_pariwisata LIKE '%$search_query%' 
                  OR p.lokasi_pariwisata LIKE '%$search_query%' 
                  OR k.nama_kategori LIKE '%$search_query%'
                  ORDER BY p.rating_pariwisata DESC";
    $query_search = mysqli_query($koneksi, $sql_search);
    $jumlah_hasil = mysqli_num_rows($query_search);
} else {
    $jumlah_hasil = 0;
}
?>
...
<head>
    ...
    <title>Hasil Pencarian | Hello Indonesia</title>
    ...
    <section class="hero">
        <div class="search-container">
            <form action="hasil_pencarian.php" method="GET" class="search-form">
                <input type="text" name="q" placeholder="Cari Pariwisata..." class="search-input" value="<?= htmlspecialchars($search_query) ?>">
    ...
    <div class="container">
        ...
        <section class="populer-section">
            <div class="kategori-header">
                <button class="kategori-btn">HASIL PENCARIAN</button>
                <h2>Menampilkan hasil untuk "<span><?= htmlspecialchars($search_query) ?></span>"</h2><br>
                <p><?= $jumlah_hasil ?> destinasi ditemukan</p>
            </div>
        </section>

        <div class="populer-grid">
            <?php if (!empty($search_query) && $jumlah_hasil > 0): ?>
                <?php while ($data = mysqli_fetch_assoc($query_search)): ?>
            ...
            <?php elseif (!empty($search_query)): ?>
                <div class="empty-search">
                    <br><br><br><br><br><br>
                    <img src="../error/error1.png" alt="error" class="empty-icon">
                </div>
            <?php else: ?>
                <div class="empty-search">
                    <br><br><br><br><br><br>
